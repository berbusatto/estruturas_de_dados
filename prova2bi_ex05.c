...
void quantidadeElementos(int *p_ultimo);
void zerarFila(int *p_ultimo, int p_Fila[10]);
...
int main() {
   ...
   while (op != 4) {
      ...
      switch (op) {
        ...
        case 5:
        	quantidadeElementos(&ultimo);
        	break;
         case 6: 
         	zerarFila(ultimo, Fila);
         	break;
...
void zerarFila(int *p_ultimo, int p_Fila[10]){
	...
	for (i = p_ultimo; i = 0; i){
		printf("Numero %d apagado", p_Fila[i]);
		*p_Fila[i] = NULL;
	}
}
